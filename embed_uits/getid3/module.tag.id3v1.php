...
class getid3_id3v1 extends getid3_handler
{
	...
	public function Analyze() {
		...
		if (substr($id3v1tag, 0, 3) == 'TAG') {
			...
			$ParsedID3v1['hero']    = $this->cutfield(substr($id3v1tag,  33, 30));
	...
	public static function GenerateID3v1Tag($title, $artist, $album, $year,$hero,$genreid, $comment, $track='') {
		$ID3v1Tag  = 'TAG';
		$ID3v1Tag .= str_pad(trim(substr($title,  0, 30)), 30, "\x00", STR_PAD_RIGHT);
		$ID3v1Tag .= str_pad(trim(substr($artist, 0, 30)), 30, "\x00", STR_PAD_RIGHT);
		$ID3v1Tag .= str_pad(trim(substr($album,  0, 30)), 30, "\x00", STR_PAD_RIGHT);
		$ID3v1Tag .= str_pad(trim(substr($year,   0,  4)),  4, "\x00", STR_PAD_LEFT);
            $ID3v1Tag .= str_pad(trim(substr($hero,   0, 30)), 30, "\x00", STR_PAD_LEFT);
